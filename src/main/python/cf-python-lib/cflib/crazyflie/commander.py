...
class Commander():
    ...
    def send_setpoint(self, roll, pitch, yaw, thrust):
        ...
        #set port to commander ()
        pk.port = CRTPPort.COMMANDER
        pk.data = struct.pack('<fffH', roll, -pitch, yaw, thrust) #<fffH means little endian, 3 floats, one unsigned short for packing

        #send pkt through the link
        self._cf.send_packet(pk)

    def send_position(self, dx, dy, height):
        ...
        pk.channel = 0
        pk.data = struct.pack('<Bffff', TYPE_POSHOLD, dx, dy, 0, height)
        self._cf.send_packet(pk)
    ...
    def send_motorValue(self, id, value):
        pk = CRTPPacket()
        pk.port = CRTPPort.DEBUGMOTOR
        pk.data = struct.pack('<HH', id, value)
        self._cf.send_packet(pk)
